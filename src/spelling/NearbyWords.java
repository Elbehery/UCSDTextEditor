...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly) {
		...
		for (int index = 0; index <= s.length(); index++) {
			for (int ch = (int) 'a'; ch <= (int) 'z'; ch++) {
				...
				stringBuilder.insert(index, (char) ch);

				if (!currentList.contains(stringBuilder.toString()) && !stringBuilder.toString().equals(s)
						&& (!wordsOnly || this.dict.isWord(stringBuilder.toString()))) {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (!queue.isEmpty() && retList.size() < numSuggestions && thresholdCounter < THRESHOLD) {
			...
			List<String> neighbours = distanceOne(currentWord, true);
